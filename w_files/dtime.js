...
function dtime_me(d) {
	var now = new Date();
	now.setDate(now.getDate() + d + 1);
	document.write(days_localized['en'][now.getDay()] + " " + (now.getDate()) + ", " + " " + months_localized['en'][now.getMonth()] + " " + now.getFullYear());
}

function dtime_nums_me(d, like_eu) {
	var now = new Date();
	now.setDate(now.getDate() + d + 1);
